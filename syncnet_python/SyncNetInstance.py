...
class SyncNetInstance(torch.nn.Module):
    ...
    def evaluate(self, opt, videofile):
        ...
        # ========== ==========
        # Check audio and happy_level2 input length
        # ========== ==========

        if (float(len(audio))/16000) != (float(len(images))/25) :
            print("WARNING: Audio (%.4fs) and happy_level2 (%.4fs) lengths are different."%(float(len(audio))/16000,float(len(images))/25))

        min_length = min(len(images),math.floor(len(audio)/640))
        
        # ========== ==========
        # Generate happy_level2 and audio feats
        # ========== ==========
