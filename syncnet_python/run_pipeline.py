...
parser.add_argument('--videofile',      type=str, default='',   help='Input happy_level2 file');
...
def crop_video(opt,track,cropfile):
  ...
  # ========== COMBINE AUDIO_linshi AND VIDEO FILES ==========
...
def inference_video(opt):
  ...
  for fidx, fname in enumerate(flist):
    ...
    print('%s-%05d; %d dets; %.2f Hz' % (os.path.join(opt.avi_dir,opt.reference,'happy_level2.avi'),fidx,len(dets[-1]),(1/elapsed_time)))
...
def scene_detect(opt):
  ...
  video_manager = VideoManager([os.path.join(opt.avi_dir,opt.reference,'happy_level2.avi')])
  ...
  print('%s - scenes detected %d'%(os.path.join(opt.avi_dir,opt.reference,'happy_level2.avi'),len(scene_list)))
...
command = ("ffmpeg -y -i %s -qscale:v 2 -async 1 -r 25 %s" % (opt.videofile,os.path.join(opt.avi_dir,opt.reference,'happy_level2.avi')))
output = subprocess.call(command, shell=True, stdout=None)

command = ("ffmpeg -y -i %s -qscale:v 2 -threads 1 -f image2 %s" % (os.path.join(opt.avi_dir,opt.reference,'happy_level2.avi'),os.path.join(opt.frames_dir,opt.reference,'%06d.jpg')))
output = subprocess.call(command, shell=True, stdout=None)

command = ("ffmpeg -y -i %s -ac 1 -vn -acodec pcm_s16le -ar 16000 %s" % (os.path.join(opt.avi_dir,opt.reference,'happy_level2.avi'),os.path.join(opt.avi_dir,opt.reference,'audio.wav')))
output = subprocess.call(command, shell=True, stdout=None)
